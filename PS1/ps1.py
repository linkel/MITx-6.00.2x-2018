...
def greedy_cow_transport(cows,limit=10):
    ...
    tripList = []
    flatTripList = []
    while len(flatTripList) < len(cows):
        subtrip = []
        current_weight = 0
        for i in range(len(sortedValues)):
            bigHolder = sortedValues[i]
            for k, v in cows.items():
                if (v == bigHolder) and (current_weight + v <= limit) and (k not in flatTripList):
                    subtrip.append(k)
...
def compare_cow_transport_algorithms():
    ...
    start = time.time()
    result = greedy_cow_transport(myDict, 10)
    end = time.time()
    print("greedy algo: " + str(end - start))
    print("trip number: " + str(len(result)))
    start = time.time()
    result = brute_force_cow_transport(myDict, 10)
    end = time.time()
    print("brute force algo: " + str(end - start))
    print("trip number: " + str(len(result)))
